...
# name of file/ or file run / this is how it initialized
app = Flask(__name__, template_folder='templates')
# we can make use of this mysql object within our post request to make an entry into the database -leen
mysql = MySQL(app)

# encrypt or secure cookies and session data related to website (not important) ~ Saad
app.config['SECRET_KEY'] = 'saad'
...
# attention to naming is unimportant, just by convention/ease
...
# GET and POST are flask HTTP methods, where GET is used to retrieve information#identify this URL accepts post and get requests, if it is a GET request, we display the form to the user, if its a POST request we store the details onto the database ~leen
...
def login():  # from a web server, and POST is used to send information
    ...
    if request.method == 'POST' and 'Email_address' in request.form and 'password' in request.form:
        ...
        with app.app_context():
            ...
            cursor.execute(
                'SELECT * FROM users WHERE Email_address = % s AND  password = %s ', (Email_address, password))
...
def logout():
    return render_template("logout.html")  # fix saad later
